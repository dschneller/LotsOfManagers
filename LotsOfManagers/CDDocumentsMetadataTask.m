...
-(BOOL)processYourThing:(CDCommand *)command result:(id)result message:(NSString *)message
{
	if (command.isFinished) { // command finished notify with result
		NSMutableDictionary* notificationPayload = [NSMutableDictionary dictionaryWithDictionary:result];
		notificationPayload[@"taskId"] = self.taskId;
		[self.delegate cdTaskDidFinish:self]; //task is finished it should be removed from the queue
        dispatch_async(dispatch_get_main_queue(), ^{
			[[NSNotificationCenter defaultCenter] postNotificationName:ELEMENT_COUNT_RETRIEVED_NOTIFICATION object:notificationPayload[@"hits"]];
            [[NSNotificationCenter defaultCenter] postNotificationName:DOCUMENTS_RETRIEVED_NOTIFICATION object:notificationPayload userInfo:nil];
        });
		return YES;
	}
	return NO;
