...
+ (NSMutableDictionary*) badgeCache
{
    static NSMutableDictionary* cache = nil;
    if (cache == nil)
    {
        //NSLog(@"Installing Cache");
        cache = [[NSMutableDictionary alloc] initWithCapacity:30];
    }
...
+ (void) clearCache
{
    ...
    [[CustomBadge badgeCache] removeAllObjects];
}
...
- (id) initWithString:(NSString *)badgeString withScale:(CGFloat)scale withShining:(BOOL)shining
{
	...
	if(self!=nil) {
		...
		self.badgeInsetColor = [UIColor blueColor];
...
+ (UIImage*) customBadgeImageWithString:(NSString *)badgeString
{
    ...
    if (!img)
    {
        ...
        UIGraphicsBeginImageContextWithOptions(badge.bounds.size, NO, [[UIScreen mainScreen] scale]);
